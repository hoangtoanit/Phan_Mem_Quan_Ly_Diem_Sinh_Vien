...
public class M_TaiKhoan {
    ...
    //Hàm xử lý chức năng sửa thông tin tài khoản
    public boolean Update_TaiKhoan(TaiKhoan TK) {
        try {
            Connection conn = null;
            conn = ConnectionDB.getConnectionDB();

            PreparedStatement updateTaiKhoan = conn.prepareStatement("UPDATE TaiKhoan SET Password = ? Where Username = ?");
            updateTaiKhoan.setString(1, TK.getPassword());
            updateTaiKhoan.setString(2, TK.getUsername());
            updateTaiKhoan.execute();
            return true;
        } catch (Exception ex) {
             Logger.getLogger(M_TaiKhoan.class.getName()).log(Level.SEVERE, null, ex);
        }
